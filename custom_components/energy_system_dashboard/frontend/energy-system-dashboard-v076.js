const ENERGY_SYSTEM_DASHBOARD_VERSION = "0.7.6";
    ...
    return `
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* V0.7.6 SYSTEM rails: static topology, exact shared junction geometry. */
        ...
        .combined-building-new .combined-floor-content { min-height:0; padding:18px 20px; display:grid; grid-template-columns:repeat(auto-fit,minmax(min(250px,100%),340px)); align-items:start; justify-content:center; gap:14px; border:0; background-color:#12161a; background-image:linear-gradient(to right,rgba(255,255,255,.025) 1px,transparent 1px),linear-gradient(to bottom,rgba(255,255,255,.025) 1px,transparent 1px); background-size:calc(100% / 12) 100%,100% 42px; }
        ...
        /* Parent and children form one intrinsic vertical column. No offset connector geometry. */
        .combined-building-new .area-child-grid,
        .combined-building-new .nested-area-group > .area-child-grid { display:grid; grid-template-columns:1fr; gap:8px; width:100%; box-sizing:border-box; margin:8px 0 0; padding:0 0 0 12px; border:0; border-left:1px solid var(--line-strong); background:transparent; }
        .combined-building-new .area-child-branch { min-width:0; width:100%; box-sizing:border-box; }
        .combined-building-new .area-child-branch::before,
        .combined-building-new .area-child-branch::after,
        .combined-building-new .nested-area-group > .area-child-grid .area-child-branch::before,
        ...
        .flow-rail-cell { position:relative; min-width:0; --rail-base:rgba(230,176,75,.24); --rail-live:#f0c96c; }
        .flow-rail-cell::before,
        .flow-rail-cell.active::after { content:""; position:absolute; width:var(--rail-width); left:calc(50% - (var(--rail-width) / 2)); background:var(--rail-base); }
        ...
        /* Electric: source above, stop at the final floor junction. */
        ...
        /* Thermal: source below, stop at the top floor junction. */
        .thermal-rail-cell.connected.first::before,
        .thermal-rail-cell.connected.first.active::after { top:var(--junction-y); bottom:0; }

        .electric-rail-cell { --rail-color:var(--active); }
        .thermal-rail-cell { --rail-color:var(--thermal); --rail-base:rgba(93,159,198,.26); --rail-live:#86c4e8; }
        .flow-branch { position:absolute; top:calc(var(--junction-y) - (var(--rail-width) / 2)); height:var(--rail-width); background:var(--rail-base); }
        .electric-branch { left:50%; right:0; }
        .thermal-branch { left:0; right:50%; }
        .flow-branch.active { background:var(--rail-live); }

        /* Entry elbows share the same X/Y constants as the floor rails. */
        .combined-entry-new { height:44px; display:grid; grid-template-columns:var(--route-gutter) minmax(0,1fr) var(--route-gutter); position:relative; --rail-base:rgba(230,176,75,.24); --rail-live:#f0c96c; }
        ...
        .combined-entry-new.electric { --rail-color:var(--active); }
        .combined-entry-new.thermal { --rail-color:var(--thermal); --rail-base:rgba(93,159,198,.26); --rail-live:#86c4e8; }
        .combined-entry-new.electric .entry-center { top:0; height:var(--junction-y); }
        .combined-entry-new.electric .entry-run { left:calc(var(--route-gutter) / 2); right:50%; }
        .combined-entry-new.electric .entry-rail { left:calc((var(--route-gutter) / 2) - (var(--rail-width) / 2)); top:var(--junction-y); bottom:0; }
        .combined-entry-new.thermal .entry-rail { right:calc((var(--route-gutter) / 2) - (var(--rail-width) / 2)); top:0; height:var(--junction-y); }
        .combined-entry-new.thermal .entry-run { left:50%; right:calc(var(--route-gutter) / 2); }
        .combined-entry-new.thermal .entry-center { top:var(--junction-y); bottom:0; }

        /* SYSTEM is deliberately static: no legacy wire/dash animation survives in this view. */
        ...
        .new-system-layout.flow-static .entry-svg-flow.active { animation:none !important; display:none !important; }

        .combined-building-new .root-area-group > .area-tile { flex-basis:auto; min-height:104px; height:auto; overflow:visible; }
        .combined-building-new .area-tile { height:auto; overflow:visible; }
        .combined-building-new .area-tile-power { margin-top:10px; padding-top:0; line-height:1.15; overflow:visible; }
        ...
        @media (max-width: 760px) {
          ...
          .flow-branch { top:21px; }
        }
      </style>`;
  }
}

if (!customElements.get("energy-system-dashboard-panel-v076")) {
  customElements.define("energy-system-dashboard-panel-v076", EnergySystemDashboardPanel);
}
