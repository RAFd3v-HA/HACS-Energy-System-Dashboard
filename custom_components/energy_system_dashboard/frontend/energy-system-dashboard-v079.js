const ENERGY_SYSTEM_DASHBOARD_VERSION = "0.7.9";
...
class EnergySystemDashboardPanel extends HTMLElement {
  ...
  _renderGenerationGroup(modules) {
    ...
    const powerValues = modules.map((module) => this._powerW(module.power_entity)).filter((value) => value !== null);
  ...
  async _onClick(event) {
    ...
    if (action === "add-generation") {
      this._draft.generation.push({ id: this._id("gen"), type: "solar", name: "PV", power_entity: "", energy_entity: "", reduction_entity: "", reduction_active_states: "on,true,1,active,reduced,begrenzt" });
      this._render();
      return;
    }
...
if (!customElements.get("energy-system-dashboard-panel-v079")) {
  customElements.define("energy-system-dashboard-panel-v079", EnergySystemDashboardPanel);
}
