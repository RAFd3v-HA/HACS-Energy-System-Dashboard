const ENERGY_SYSTEM_DASHBOARD_VERSION = "0.7.10";
...
class EnergySystemDashboardPanel extends HTMLElement {
  constructor() {
    ...
    this._dragLevelId = null;
    this._expandedParents = new Set();
    ...
    this._boundDragEnd = () => { this._dragAreaId = null; this._dragOrigin = null; this._dragLevelId = null; };
    this._combinedFlowResizeObserver = null;
    this._combinedFlowFrame = null;
  }
  ...
  set cardConfig(value) {
    ...
    this._cardConfig = value ? {
      ...
      expand_children: value.expand_children === true,
    } : null;
  ...
  disconnectedCallback() {
    ...
    this._calculationUnsub = null;
    if (this._combinedFlowResizeObserver) this._combinedFlowResizeObserver.disconnect();
    this._combinedFlowResizeObserver = null;
    if (this._combinedFlowFrame) cancelAnimationFrame(this._combinedFlowFrame);
    this._combinedFlowFrame = null;
  }
  ...
  _capacityKWh(entityId) {
    return this._energyStateKWh(entityId);
  }

  _areaCurrentTemperatures(area) {
    if (!area) return [];
    const configured = Array.isArray(area.current_temperature_entities)
      ? area.current_temperature_entities
      : [];
    const legacy = area.current_temperature_entity ? [area.current_temperature_entity] : [];
    return [...new Set([...configured, ...legacy].filter(Boolean))]
      .map((entityId) => this._temperature(entityId))
  ...
  _areaClimate(area, config = this._config) {
    if (!area) return { current: null, target: null };
    const sensorAverage = this._average(this._areaCurrentTemperatures(area));
    if (area.climate_mode === "climate" && area.climate_entity) {
      ...
      const available = state && !["unknown", "unavailable", "none", ""].includes(climateState);
      ...
      const current = sensorAverage !== null ? sensorAverage : (available && Number.isFinite(currentAttr) ? currentAttr : null);
      const target = !available || isOff ? null : (targetOverride !== null ? targetOverride : (Number.isFinite(targetAttr) ? targetAttr : null));
      return { current, target };
    }
    if (area.climate_mode === "entities") {
      return { current: sensorAverage, target: this._temperature(area.target_temperature_entity) };
    }
    return { current: null, target: null };
  }

  _branchClimateCurrent(area, config = this._config, stack = new Set()) {
    if (!area || stack.has(area.id)) return [];
    const next = new Set(stack); next.add(area.id);
    const own = this._areaClimate(area, config).current;
    const values = own === null ? [] : [own];
    for (const child of this._sameLevelChildren(area.id, area.level_id, config)) values.push(...this._branchClimateCurrent(child, config, next));
    return values;
  }

  _areaAverageRoomTemperature(area, config = this._config) {
    const values = this._branchClimateCurrent(area, config).filter(Number.isFinite);
    return values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : null;
  }

  _levelTemperatureValues(level, field, config = this._config) {
    if (!level) return [];
    return (config?.areas || []).filter((area) => area.id !== "house" && area.level_id === level.id && area[field])
      .map((area) => this._temperature(area[field])).filter((value) => value !== null && Number.isFinite(value));
  }

  _levelSupplyTemperature(level, config = this._config) {
    const values = this._levelTemperatureValues(level, "supply_temperature_entity", config);
    return values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : null;
  }

  _levelReturnTemperature(level, config = this._config) {
    const values = this._levelTemperatureValues(level, "return_temperature_entity", config);
    return values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : null;
  }

  _levelAverageRoomTemperature(level, config = this._config) {
    if (!level) return null;
    const values = this._visualRootAreas(level.id, config).flatMap((area) => this._branchClimateCurrent(area, config)).filter(Number.isFinite);
    return values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : null;
  }

  _isReducedGeneration(module) {
    const reductionRaw = String(this._state(module?.reduction_entity)?.state || "").toLowerCase();
    const reducedStates = String(module?.reduction_active_states || "on,true,1,active,reduced,begrenzt")
      .split(",").map((item) => item.trim().toLowerCase()).filter(Boolean);
    return Boolean(module?.reduction_entity && reducedStates.includes(reductionRaw));
  }

  _renderSourceSummary(kind, modules) {
  ...
  _pvProductionThresholdW(module) {
    const configured = Number(module?.production_threshold_w ?? 50);
    return Number.isFinite(configured) && configured >= 0 ? configured : 50;
  }

  _renderGenerationGroup(modules) {
    if (!modules.length) return "";
    const moduleStates = modules.map((module) => {
      const power = this._powerW(module.power_entity);
      const threshold = this._pvProductionThresholdW(module);
      return { module, power, threshold, active: power !== null && power >= threshold };
    });
    const powerValues = moduleStates.map((item) => item.power).filter((value) => value !== null);
    const totalPower = powerValues.length ? powerValues.reduce((sum, value) => sum + value, 0) : null;
    const totalTodayValues = modules.map((module) => this._dailyEnergyKWh(module.energy_entity)).filter((value) => value !== null);
    const totalToday = totalTodayValues.length ? totalTodayValues.reduce((sum, value) => sum + value, 0) : null;
    const active = moduleStates.some((item) => item.active);
    const reduced = moduleStates.some((item) => item.active && this._isReducedGeneration(item.module));
    ...
    const status = totalPower === null ? "unknown" : reduced && active ? "reduced" : active ? "good" : "idle";
  ...
  _renderBatteryGroup(modules) {
    if (!modules.length) return "";
    const powers = modules.map((module) => this._powerW(module.power_entity)).filter((value) => value !== null);
    const totalPower = powers.length ? powers.reduce((sum, value) => sum + value, 0) : null;
    const capacities = modules.map((module) => this._capacityKWh(module.capacity_entity) ?? (Number(module.capacity_kwh) > 0 ? Number(module.capacity_kwh) : null));
    const validCaps = capacities.filter((value) => value !== null && value > 0);
    const totalCapacity = validCaps.length ? validCaps.reduce((sum, value) => sum + value, 0) : null;
    const socPairs = modules.map((module, index) => ({ soc:this._numeric(module.soc_entity), cap:capacities[index] })).filter((item) => item.soc !== null);
    const weighted = socPairs.filter((item) => item.cap !== null && item.cap > 0);
    let totalSoc = null;
    if (weighted.length) totalSoc = weighted.reduce((sum, item) => sum + item.soc * item.cap, 0) / weighted.reduce((sum, item) => sum + item.cap, 0);
    ...
    const status = totalSoc === null ? "unknown" : totalSoc < 10 ? "fault" : totalSoc < 30 ? "low" : totalSoc < 90 ? "active" : "good";
  ...
  _entityList(kind = "any") {
    ...
    return states
      .filter((state) => {
        ...
        if (kind === "climate") return domain === "climate";
        return true;
      })
  ...
  _moduleStatus(module) {
    const explicitStatus = module.status_mode === "entity" || (module.status_entity && module.status_mode !== "power");
    if (explicitStatus && module.status_entity) {
      const rawOriginal = String(this._state(module.status_entity)?.state || "");
      const raw = rawOriginal.toLowerCase();
      if (["unavailable", "unknown", ""].includes(raw)) return { cls: "unknown", text: "UNKNOWN" };
      if (["fault", "error", "störung", "alarm"].some((token) => raw.includes(token))) return { cls: "fault", text: rawOriginal.toUpperCase() };
      const heatingStates = String(module.heating_states || "heating,active,on,1,ein")
        .split(",").map((item) => item.trim().toLowerCase()).filter(Boolean);
      if (module.type === "electric_heater") {
        return heatingStates.includes(raw) ? { cls: "active", text: "HEATING" } : { cls: "idle", text: "STANDBY" };
      }
      if (["off", "standby", "idle", "0", "false", "aus"].includes(raw)) return { cls: "idle", text: rawOriginal.toUpperCase() };
      return { cls: "active", text: rawOriginal.toUpperCase() };
    }
    const power = this._powerW(module.power_entity || module.thermal_power_entity);
    if (power === null) return { cls: "unknown", text: "UNKNOWN" };
    const threshold = module.type === "electric_heater" ? Number(module.heating_power_threshold_w ?? 100) : 10;
    const active = Math.abs(power) >= (Number.isFinite(threshold) ? threshold : 100);
    return active ? { cls: "active", text: module.type === "electric_heater" ? "HEATING" : "ACTIVE" } : { cls: "idle", text: "STANDBY" };
  }
  ...
  _gridFlow() {
    ...
    return {
      // Normalize the displayed grid sign independently of the source sensor:
      // grid import is positive, grid export is negative.
      power: importing ? Math.abs(raw) : -Math.abs(raw),
  ...
  _sortedLevels(config = this._config) {
    return [...(config?.levels || [])].sort((a, b) => Number(a.order || 0) - Number(b.order || 0) || String(a.name || "").localeCompare(String(b.name || ""), "de"));
  }
  ...
  _buildingLevels(config = this._config, includeEmpty = false) {
    ...
    return this._sortedLevels(config).filter((level) => includeEmpty || areaLevelIds.has(level.id));
  }
  ...
  _house(config = this._config) {
    return this._houseArea(config);
  }
  ...
  _isConfiguredGeneration(module) {
    return Boolean(module && [module.power_entity, module.energy_entity, module.reduction_entity].some(Boolean));
  }
  ...
  _isConfiguredHeating(module) {
    return Boolean(module && [
      ...
      module.target_temperature_entity,
    ].some(Boolean));
  }
  ...
  _render() {
    ...
    this._scheduleCombinedFlowLayout();
  }
  ...
  _electricalNodes() {
    ...
    if (this._isConfiguredGrid(config)) {
      const currentImport = gridFlow.mode === "import" && gridFlow.power !== null ? Math.abs(gridFlow.power) : 0;
      const currentExport = gridFlow.mode === "export" && gridFlow.power !== null ? -Math.abs(gridFlow.power) : 0;
      nodes.push(this._node({ code:"GRID", name:config.grid.name || "Netz", main:this._formatPowerW(gridFlow.power), state:gridFlow.label, status:gridFlow.mode === "unknown" ? "unknown" : gridFlow.mode === "import" ? "active" : "good", custom:`<div class="mini-line grid-current"><span>BEZUG AKTUELL</span><strong>${this._formatPowerW(currentImport)}</strong></div><div class="mini-line grid-current"><span>EINSPEISUNG AKTUELL</span><strong>${this._formatPowerW(currentExport)}</strong></div><div class="mini-line daily-value"><span>BEZUG HEUTE</span><strong>${this._formatEnergyKWh(this._dailyEnergyKWh(config.grid.import_energy_entity))}</strong></div><div class="mini-line daily-value"><span>EINSPEISUNG HEUTE</span><strong>${this._formatEnergyKWh(this._dailyEnergyKWh(config.grid.export_energy_entity))}</strong></div>`, metaLeft:"REFERENCE", metaRight:config.grid.direction === "export_positive" ? "EXPORT +" : "IMPORT +" }));
    }
    const solar = (config.generation || []).filter((item) => item.type === "solar" && this._isConfiguredGeneration(item));
    const other = (config.generation || []).filter((item) => item.type !== "solar" && this._isConfiguredGeneration(item));
    const pvNode = this._renderGenerationGroup(solar); if (pvNode) nodes.push(pvNode);
    for (const module of other) { const power=this._powerW(module.power_entity); nodes.push(this._node({ code:"GEN", name:module.name || "Erzeuger", main:this._formatPowerW(power), state:power !== null && power > 10 ? "PRODUCTION" : power === null ? "UNKNOWN" : "IDLE", status:power === null ? "unknown" : power > 10 ? "good" : "idle", custom:module.energy_entity ? `<div class="mini-line daily-value"><span>HEUTE</span><strong>${this._formatEnergyKWh(this._dailyEnergyKWh(module.energy_entity))}</strong></div>` : "", metaLeft:"SOURCE", metaRight:"GENERATION" })); }
    const batteryNode = this._renderBatteryGroup((config.storage || []).filter((item) => this._isConfiguredStorage(item))); if (batteryNode) nodes.push(batteryNode);
    return nodes;
  }

  _combinedFloorFlags(level, config = this._config) {
    const roots = this._visualRootAreas(level.id, config);
    const visible = roots.some((area) => this._branchHasConfig(area, "electric", config) || this._branchHasConfig(area, "thermal", config) || this._branchClimateCurrent(area, config).length);
    // The visible building topology is static: every rendered floor is connected to both rails.
    // Only the live current-power state decides whether a branch/segment is highlighted.
    return { electric: visible, thermal: visible, visible };
  }

  _renderCombinedFloorGroup(level, config = this._config, route = {}) {
    const roots = this._visualRootAreas(level.id, config).filter((area) => this._branchHasConfig(area, "electric", config) || this._branchHasConfig(area, "thermal", config) || this._branchClimateCurrent(area, config).length);
    ...
    const electricClasses=["flow-rail-cell","electric-rail-cell",route.electricRail?"connected":"",route.electricFirst?"first":"",route.electricLast?"last":"",route.electricRailActive?"active":"",route.electricActiveEnd?"live-end":""].filter(Boolean).join(" ");
    const thermalClasses=["flow-rail-cell","thermal-rail-cell",route.thermalRail?"connected":"",route.thermalFirst?"first":"",route.thermalLast?"last":"",route.thermalRailActive?"active":"",route.thermalActiveEnd?"live-end":""].filter(Boolean).join(" ");
    const thermalDetails=[supply===null?"":`<span>VL <strong>${this._formatTemp(supply)}</strong></span>`,ret===null?"":`<span>RL <strong>${this._formatTemp(ret)}</strong></span>`,delta===null?"":`<span>ΔT <strong>${delta.toLocaleString("de-DE",{maximumFractionDigits:1})} K</strong></span>`,thermalPower===null?"":`<span>TH. <strong>${this._formatPowerW(thermalPower)}</strong></span>`,thermalToday===null?"":`<span class="daily-value">HEUTE <strong>${this._formatEnergyKWh(thermalToday)}</strong></span>`].join("");
    return `<section class="combined-floor-row">
      <div class="${electricClasses}" aria-hidden="true">${route.electricBranch ? `<i class="flow-branch electric-branch ${route.electricBranchActive?"active":""}"></i>` : ""}</div>
      <div class="combined-floor-card">
        <header class="combined-floor-header"><div class="floor-title"><strong>${this._esc(level.name)}</strong>${electricPower===null?"":`<span class="floor-power">${this._formatPowerW(electricPower)}</span>`}</div><div class="floor-meta">${electricToday===null?"":`<span class="daily-value">HEUTE ${this._formatEnergyKWh(electricToday)}</span>`}${room===null?"":`<span>Ø RAUM ${this._formatTemp(room)}</span>`}${thermalDetails?`<span class="thermal-floor-meta">${thermalDetails}</span>`:""}</div></header>
        <div class="combined-floor-content">${roots.map((area)=>this._renderAreaGroup(area,config,false,"combined")).join("")}</div>
      </div>
      <div class="${thermalClasses}" aria-hidden="true">${route.thermalBranch ? `<i class="flow-branch thermal-branch ${route.thermalBranchActive?"active":""}"></i>` : ""}</div>
    </section>`;
  }

  _combinedFlowState(config = this._config) {
    const levels=this._buildingLevels(config).filter((level)=>this._combinedFloorFlags(level,config).visible);
    const electricFlags=levels.map((level)=>this._combinedFloorFlags(level,config).electric);
    const thermalFlags=levels.map((level)=>this._combinedFloorFlags(level,config).thermal);
    const electricActive=levels.some((level)=>{ const value=this._levelPower(level,config); return value!==null && Math.abs(value)>=0.5; });
    const thermalActive=levels.some((level)=>{ const value=this._levelThermalPower(level,config); return value!==null && Math.abs(value)>=0.5; });
    return {levels,electricFlags,thermalFlags,hasElectric:electricFlags.some(Boolean),hasThermal:thermalFlags.some(Boolean),electricActive,thermalActive};
  }

  _renderCombinedEntry(kind, active = false) {
    return `<div class="combined-entry-new ${kind} ${active?"active":""}" aria-hidden="true"><i class="entry-center"></i><i class="entry-run"></i><i class="entry-rail"></i></div>`;
  }

  _renderCombinedBuilding(config = this._config, flowState = null) {
    const state=flowState || this._combinedFlowState(config), levels=state.levels;
    if (!levels.length) return this._empty("Noch keine konfigurierten Gebäudeebenen vorhanden.");
    const electricFlags=state.electricFlags || levels.map((level)=>this._combinedFloorFlags(level,config).electric);
    const thermalFlags=state.thermalFlags || levels.map((level)=>this._combinedFloorFlags(level,config).thermal);
    const electricFlow=levels.map((level)=>{ const value=this._levelPower(level,config); return value!==null && Math.abs(value)>=0.5; });
    const thermalFlow=levels.map((level)=>{ const value=this._levelThermalPower(level,config); return value!==null && Math.abs(value)>=0.5; });
    ...
    const lastElectricActive=electricFlow.lastIndexOf(true);
    const firstThermalActive=thermalFlow.indexOf(true);
    const rows=levels.map((level,index)=>this._renderCombinedFloorGroup(level,config,{
      ...
      electricRailActive:lastElectricActive>=0 && index>=firstElectric && index<=lastElectricActive,
      electricActiveEnd:lastElectricActive>=0 && index===lastElectricActive,
      ...
      thermalRailActive:firstThermalActive>=0 && index>=firstThermalActive && index<=lastThermal,
      thermalActiveEnd:firstThermalActive>=0 && index===firstThermalActive,
    ...
    return `<div class="combined-building-new">${rows}</div>`;
  }

  _scheduleCombinedFlowLayout() {}
  _layoutCombinedFlow() {}

  _renderSystem() {
    const config=this._config; this._syncLayoutState(config);
    const electrical=this._electricalNodes();
    const heating=(config.heating||[]).filter((module)=>this._isConfiguredHeating(module));
    ...
    const house=this._houseArea(config), housePower=house ? this._areaPower(house,config) : null, levelPower=this._totalLevelValue("power",config), houseLabel=this._formatPowerW(levelPower !== null ? levelPower : housePower);
    ...
    const directRoomHtml=directRoom.length ? `<div class="direct-room-zone"><div class="section-label room-label">DIREKTE RAUMWÄRME · NICHT WASSERGEFÜHRT</div><div class="system-grid module-board thermal-board">${this._gridCells(directRoom.map((module)=>this._renderHeatingNode(module,"thermal")))}</div></div>` : "";
    return `<div class="system-note">GESAMTSYSTEM · ELEKTRISCHE UND THERMISCHE FLOWS SIND PHYSIKALISCH GETRENNT</div>
      <section class="combined-system-zone new-system-layout flow-static">
        <div class="combined-energy-side electric-side">
          <div class="section-label split-label"><span>01 / ELEKTRISCH</span>${houseLabel?`<strong>AKTUELLE LEISTUNG ${houseLabel}</strong>`:""}</div>
          ${electrical.length?`<div class="system-grid module-board">${this._gridCells(electrical)}</div>${this._flowWire("electric",24)}`:""}
          <div class="bus closed-bus"><span>ELEKTRISCHE VERTEILUNG</span>${houseLabel?`<strong>${houseLabel}</strong>`:""}</div>
        </div>
        ${combinedFlow.hasElectric?this._renderCombinedEntry("electric",combinedFlow.electricActive):`<div class="combined-entry-spacer"></div>`}
        ${this._renderCombinedBuilding(config,combinedFlow)}
        ${combinedFlow.hasThermal?this._renderCombinedEntry("thermal",combinedFlow.thermalActive):`<div class="combined-entry-spacer"></div>`}
        <div class="combined-energy-side thermal-side">
          <div class="distribution-box thermal-distribution closed-distribution"><span>THERMISCHE VERTEILUNG</span>${thermalLabel?`<strong>${thermalLabel}</strong>`:""}</div>
          ${bufferVisible?`${this._flowWire("thermal",24)}<div class="system-grid module-board buffer-board"><div class="system-cell full-span">${this._renderBuffer(buffer)}</div></div>`:""}
          ${toBuffer.length&&bufferVisible?`${this._flowWire("thermal",24)}<div class="system-grid module-board thermal-board">${this._gridCells(toBuffer.map((module)=>this._renderHeatingNode(module,"thermal")))}</div>`:""}
          ${directRoomHtml}
        </div>
      </section>`;
  }
  ...
  _renderThermal() {
    ...
    const bufferVisible = this._isConfiguredBuffer(buffer);
    const toBuffer = bufferVisible ? heating.filter((module) => module.target === "buffer") : [];
    const toRoom = heating.filter((module) => module.target !== "buffer" || !bufferVisible);
    ...
    return `
      ...
      ${bufferNodes.length && bufferVisible ? this._flowWire("thermal", 28) : ""}
      ${bufferVisible ? `<div class="system-grid module-board buffer-board"><div class="system-cell full-span">${this._renderBuffer(buffer)}</div></div>` : ""}
      ${levels.length ? `${bufferVisible ? this._flowWire("thermal", 28) : ""}<div class="distribution-box thermal-distribution"><span>THERMISCHE VERTEILUNG</span><strong>${this._thermalLoadLabel()}</strong></div>${this._renderBuildingStack(this._config, null, false, "thermal")}` : ""}
  ...
  _renderHeatingNode(module, context = "electric") {
    ...
    if (module.target_temperature_entity) details.push(["ZIEL", this._formatTemp(this._temperature(module.target_temperature_entity))]);
    ...
    return this._node({
      ...
      metaLeft: module.target === "buffer" && this._isConfiguredBuffer(this._config?.buffer) ? "→ BUFFER" : module.target !== "buffer" ? "→ ROOM" : "",
  ...
  _renderEditorFloorGroup(level, config = this._draft || this._config) {
    ...
    return `
      <section class="editor-floor-group" data-floor-group="${this._esc(level.id)}" data-level-drop="${this._esc(level.id)}">
        <aside class="floor-indicator level-drag-handle" draggable="true" data-drag-level="${this._esc(level.id)}" title="Stockwerk per Drag & Drop verschieben"><strong>${this._esc(level.name)}</strong><span class="floor-load">${this._formatPowerW(this._levelPower(level, config))}</span><small>${areas.length} BEREICHE</small></aside>
        <div class="floor-layout-body">
  ...
  _parentExpanded(areaId, interactive = false) {
    if (interactive) return true;
    if (this._cardConfig?.expand_children === true) return true;
    return this._expandedParents.has(areaId);
  }

  _renderAreaGroup(area, config = this._config, interactive = false, kind = "electric", depth = 0, nested = false) {
    const children = this._sameLevelChildren(area.id, area.level_id, config)
      .filter((child) => interactive || (kind === "combined"
        ? this._branchHasConfig(child, "electric", config) || this._branchHasConfig(child, "thermal", config) || this._branchClimateCurrent(child, config).length
        : this._branchHasConfig(child, kind, config)));
    const layout = this._effectiveLayout(area, config);
    const tileLayout = this._clampLayout(area.layout);
    const expanded = children.length ? this._parentExpanded(area.id, interactive) : false;
    const groupClass = children.length ? `has-children ${expanded ? "is-expanded" : "is-collapsed"}` : "leaf-group";
    const position = nested
      ? `style="--root-tile-height:${Math.max(42, Number(tileLayout.h || 1) * 42)}px"`
      : `style="grid-column:${Number(layout.x || 1)} / span ${Number(layout.w || 3)};grid-row:${Number(layout.y || 1)} / span ${Number(layout.h || 2)};--area-span:${Number(layout.w || 3)};--root-tile-height:${Math.max(42, Number(tileLayout.h || 1) * 42)}px"`;
    return `<div class="area-group ${nested ? "nested-area-group" : "root-area-group"} ${groupClass} depth-${Math.min(depth, 3)}" data-area-group="${this._esc(area.id)}" ${position}>
      ...
      ${children.length && expanded ? `<div class="area-child-grid" data-child-drop-parent="${this._esc(area.id)}">${children.map((child, index) => `<div class="area-child-branch ${index === children.length - 1 ? "last-child-branch" : ""}" data-child-branch="${this._esc(child.id)}">${this._renderAreaGroup(child, config, interactive, kind, depth + 1, true)}</div>`).join("")}</div>` : ""}
    </div>`;
  }
  ...
  _renderAreaTile(area, config = this._config, interactive = false, kind = "electric", depth = 0) {
    ...
    return `<${tag} class="area-tile ${kind === "thermal" ? "thermal-area" : isCombined ? "combined-area" : "electric-area"} ${this._areaUsesCalculation(area) ? "calculated" : "measured"} layout-${this._layoutMode(area)}${hierarchyClass} ${selected ? "selected" : ""}" ${attrs}><div class="area-tile-head"><span>${this._areaUsesCalculation(area) ? "C" : "M"}</span><strong>${this._esc(area.name)}</strong></div>${relation}<div class="area-tile-power">${this._formatPowerW(power)}</div><div class="area-tile-energy daily-value"><span>HEUTE</span><strong>${this._formatEnergyKWh(today)}</strong></div>${roomHtml}${thermalHtml}${isCombined ? "" : `<div class="area-tile-formula">${this._esc(this._areaFormula(area,config,formulaKind))}</div>`}</${tag}>`;
  }
      ...
      <section class="config-card">
        ...
        ${(d.generation || []).map((module, index) => `
          <div class="module-editor">
            ...
            ${module.type === "solar" ? this._field("Produktion ab / W", `<input type="number" min="0" step="1" data-array="generation" data-index="${index}" data-field="production_threshold_w" value="${Number(module.production_threshold_w ?? 50)}">`) : ""}
            ${module.type === "solar" ? this._field("Netzbetreiber-Begrenzung", `<select data-array="generation" data-index="${index}" data-field="reduction_entity">${this._entityOptions("any", module.reduction_entity)}</select>`) : ""}
            ${module.type === "solar" ? this._field("Begrenzt bei Zuständen", `<input data-array="generation" data-index="${index}" data-field="reduction_active_states" value="${this._esc(module.reduction_active_states || "on,true,1,active,reduced,begrenzt")}">`) : ""}
      ...
      <section class="config-card">
        ...
        ${(d.storage || []).map((module, index) => `
          <div class="module-editor">
            ...
            ${this._field("Kapazitäts-Entity", `<select data-array="storage" data-index="${index}" data-field="capacity_entity">${this._entityOptions("energy", module.capacity_entity)}</select>`)}
            ${this._field("Kapazität manuell / kWh", `<input type="number" min="0" step="0.1" data-array="storage" data-index="${index}" data-field="capacity_kwh" value="${Number(module.capacity_kwh || 0)}">`)}
        ...
        <div class="module-grid">
          ${(d.heating || []).map((module, index) => `
            <div class="module-editor">
              ...
              ${module.type === "electric_heater" ? this._field("Statusermittlung", `<select data-array="heating" data-index="${index}" data-field="status_mode"><option value="power" ${module.status_mode !== "entity" ? "selected" : ""}>Automatisch aus Leistungsaufnahme</option><option value="entity" ${module.status_mode === "entity" ? "selected" : ""}>Status-Entity verwenden</option></select>`) : ""}
              ${module.type !== "electric_heater" || module.status_mode === "entity" ? this._field("Status", `<select data-array="heating" data-index="${index}" data-field="status_entity">${this._entityOptions("any", module.status_entity)}</select>`) : this._field("Schwellwert HEATING / W", `<input type="number" min="0" step="1" data-array="heating" data-index="${index}" data-field="heating_power_threshold_w" value="${Number(module.heating_power_threshold_w ?? 100)}">`)}
              ${module.type === "electric_heater" && module.status_mode === "entity" ? this._field("HEATING bei Zuständen", `<input data-array="heating" data-index="${index}" data-field="heating_states" value="${this._esc(module.heating_states || "heating,active,on,1,ein")}">`) : ""}
              ...
              ${module.type === "electric_heater" ? this._field("Zieltemperatur", `<select data-array="heating" data-index="${index}" data-field="target_temperature_entity">${this._entityOptions("temperature", module.target_temperature_entity)}</select>`) : ""}
  ...
  async _onClick(event) {
    ...
    if (action === "toggle-parent") {
      ...
      if (this._expandedParents.has(areaId)) this._expandedParents.delete(areaId);
      else this._expandedParents.add(areaId);
      this._render();
      return;
    }
    ...
    if (action === "add-generation") {
      this._draft.generation.push({ id: this._id("gen"), type: "solar", name: "PV", power_entity: "", energy_entity: "", production_threshold_w: 50, reduction_entity: "", reduction_active_states: "on,true,1,active,reduced,begrenzt" });
      this._render();
      return;
    }
    if (action === "add-storage") {
      this._draft.storage.push({ id: this._id("bat"), type: "battery", name: "Teilspeicher", power_entity: "", soc_entity: "", charge_energy_entity: "", discharge_energy_entity: "", capacity_entity: "", capacity_kwh: 0 });
      this._render();
      return;
    }
    if (action === "add-heating") {
      this._draft.heating.push({ id: this._id("heat"), type: "heatpump", name: "Wärmepumpe", target: "buffer", status_mode: "entity", status_entity: "", heating_power_threshold_w: 100, heating_states: "heating,active,on,1,ein", power_entity: "", energy_entity: "", thermal_power_entity: "", thermal_energy_entity: "", supply_entity: "", return_entity: "", temperature_entity: "", target_temperature_entity: "" });
      this._render();
      return;
    }
    if (action === "add-area") {
  ...
  _onChange(event) {
    ...
    if (target.dataset.areaId) {
      ...
      if (!area) return;
      if (target.dataset.areaTempMulti !== undefined) {
        area.current_temperature_entities = [...target.selectedOptions].map((option) => option.value).filter(Boolean);
        area.current_temperature_entity = "";
        this._saveNotice = "";
        return;
      }
      ...
      if (target.dataset.areaField) {
        ...
        if (["mode", "calculation_type", "level_id", "name", "basis_area_id", "parent_id", "climate_mode"].includes(target.dataset.areaField)) this._render();
      }
    }
  }

  _onDragStart(event) {
    const levelHandle = event.target.closest("[data-drag-level]");
    if (levelHandle) {
      this._dragLevelId = levelHandle.dataset.dragLevel;
      event.dataTransfer?.setData("application/x-energy-level", this._dragLevelId);
  ...
  _onDragOver(event) {
    if (event.target.closest("[data-level-drop]") || event.target.closest("[data-layout-grid]") || event.target.closest("[data-child-drop-parent]")) event.preventDefault();
  }

  _onDrop(event) {
    if (!this._draft) return;
    const levelDrop = event.target.closest("[data-level-drop]");
    const draggedLevelId = event.dataTransfer?.getData("application/x-energy-level") || this._dragLevelId;
    if (levelDrop && draggedLevelId) {
      event.preventDefault();
      const levels = this._sortedLevels(this._draft);
      const from = levels.findIndex((level) => level.id === draggedLevelId);
      let to = levels.findIndex((level) => level.id === levelDrop.dataset.levelDrop);
      if (from >= 0 && to >= 0 && from !== to) {
        const [moved] = levels.splice(from, 1);
        if (from < to) to -= 1;
        const rect = levelDrop.getBoundingClientRect();
        if (event.clientY > rect.top + rect.height / 2) to += 1;
        levels.splice(Math.max(0, Math.min(levels.length, to)), 0, moved);
        levels.forEach((level, index) => { level.order = index; });
        this._draft.levels = levels;
        this._saveNotice = "STOCKWERKSREIHENFOLGE GEÄNDERT · NOCH SPEICHERN";
      }
      this._dragLevelId = null;
      this._render();
      return;
    ...
    return `
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        .node.status-reduced::before { animation:reduced-status 1.1s steps(1,end) infinite; }
        .node.status-reduced .status i { animation:reduced-status 1.1s steps(1,end) infinite; box-shadow:0 0 0 4px rgba(226,183,89,.1); }
        @keyframes reduced-status { 0%,49% { background:var(--good); } 50%,100% { background:#e89a43; } }
        .node.status-low::before { background:var(--low); }
        ...
        .status-low .status i { background:var(--low); }
        ...
        .level-drag-handle { cursor:grab; user-select:none; }
        ...
        .area-group.is-collapsed > .area-tile.hierarchy-parent { cursor:pointer; }
        .area-group.is-expanded > .area-tile.hierarchy-parent { cursor:pointer; }
        ...
        .source-summary { margin-top:8px; border-top:1px solid var(--line); }
        .source-summary-row { min-height:30px; display:grid; grid-template-columns:minmax(0,1fr) auto auto; gap:10px; align-items:center; padding:5px 0; font:650 9px/1 ui-monospace,monospace; }
        .source-summary-row + .source-summary-row { border-top:1px solid rgba(255,255,255,.055); }
        .source-summary-row span { min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:var(--muted); }
        ...
        .combined-energy-side > .bus, .combined-energy-side > .distribution-box { width:100%; max-width:1500px; margin-left:auto; margin-right:auto; }
        .flow-static .flow-segment::after, .flow-static.flow-segment::after { display:none !important; }
        .combined-system-zone { display:flex; flex-direction:column; gap:0; }
        .combined-energy-side { position:relative; }
        .combined-entry { height:48px; position:relative; max-width:1500px; margin:0 auto; overflow:visible; }
        .combined-entry-spacer { height:32px; max-width:1500px; margin:0 auto; }
        .combined-entry .entry-drop, .combined-entry .entry-rise { left:50%; width:2px; }
        .combined-entry .entry-run { height:2px; }
        .electric-entry .entry-drop { top:0; bottom:0; }
        .electric-entry .entry-run { left:24px; right:50%; bottom:0; top:auto; }
        .thermal-entry .entry-rise { top:0; bottom:0; }
        ...
        .floor-junction-link { display:none !important; }
        .floor-to-level-line { display:block; width:1px; height:18px; margin:0 auto; background:var(--line-strong); }
        .combined-floor-content { align-self:stretch; min-width:0; min-height:86px; padding:18px 34px; display:flex; flex-wrap:wrap; justify-content:center; align-items:center; gap:0; border:1px solid var(--line); background-color:#12161a; background-image:linear-gradient(to right,rgba(255,255,255,.025) 1px,transparent 1px),linear-gradient(to bottom,rgba(255,255,255,.025) 1px,transparent 1px); background-size:calc(100% / 12) 100%,100% 42px; }
        ...
        .electric-route, .thermal-route { position:absolute; inset:0; min-width:0; overflow:visible; pointer-events:none; z-index:3; }
        .route-anchor { min-height:1px; }
        .combined-flow-overlay { position:absolute; inset:0; width:100%; height:100%; overflow:visible; pointer-events:none; z-index:5; }
        .route-svg-path { fill:none; stroke-width:2; vector-effect:non-scaling-stroke; }
        .route-electric { stroke:rgba(226,183,89,.34); }
        .route-thermal { stroke:rgba(93,159,198,.40); }
        .route-svg-path.route-active { stroke-dasharray:6 16; animation:route-dash 1.8s linear infinite; animation-delay:var(--flow-phase,0s); }
        .route-electric.route-active { stroke:var(--active); }
        .route-thermal.route-active { stroke:var(--thermal); animation-direction:reverse; }
        @keyframes route-dash { to { stroke-dashoffset:-44; } }
        .combined-entry svg { position:absolute; inset:0; width:100%; height:100%; overflow:visible; }
        .entry-svg-base, .entry-svg-flow { fill:none; stroke-width:2; vector-effect:non-scaling-stroke; }
        .entry-svg-base.electric { stroke:rgba(226,183,89,.34); }
        .entry-svg-base.thermal { stroke:rgba(93,159,198,.40); }
        .entry-svg-flow { stroke:transparent; }
        .entry-svg-flow.active { stroke-dasharray:6 16; animation:route-dash 1.8s linear infinite; }
        .entry-svg-flow.electric.active { stroke:var(--active); }
        .entry-svg-flow.thermal.active { stroke:var(--thermal); animation-direction:reverse; }
        ...
        .area-climate strong, .area-thermal-summary strong { color:var(--text); }
        .parent-climate strong { color:var(--good); }
        ...
        /* V0.7.7 SYSTEM rails: static topology, live state, exact shared junction geometry. */
        .new-system-layout { --route-gutter:58px; --rail-width:2px; --junction-y:22px; }
        .combined-building-new, .combined-entry-new { width:min(100%,1220px); max-width:1220px; margin:0 auto; }
        .combined-building-new { display:flex; flex-direction:column; }
        .combined-floor-row { display:grid; grid-template-columns:var(--route-gutter) minmax(0,1fr) var(--route-gutter); align-items:stretch; min-width:0; }
        .combined-floor-card { grid-column:2; min-width:0; border-left:1px solid var(--line-strong); border-right:1px solid var(--line-strong); border-top:1px solid var(--line-strong); background:#0e1216; }
        .combined-floor-row:last-child .combined-floor-card { border-bottom:1px solid var(--line-strong); }
        .combined-floor-header { min-height:44px; display:flex; align-items:center; justify-content:space-between; gap:12px; padding:0 14px; border-bottom:1px solid var(--line-strong); background:#101418; }
        .floor-title, .floor-meta, .thermal-floor-meta { display:flex; align-items:center; gap:10px; min-width:0; }
        .floor-title strong { font:800 12px/1 ui-monospace,monospace; letter-spacing:.08em; }
        .floor-power { color:var(--active); font:850 12px/1 ui-monospace,monospace; }
        .floor-meta { justify-content:flex-end; flex-wrap:wrap; color:var(--muted); font:700 8px/1 ui-monospace,monospace; }
        .thermal-floor-meta { color:var(--thermal); }
        .thermal-floor-meta strong { color:var(--text); }
        .combined-building-new .combined-floor-content { min-height:0; padding:12px 16px; display:grid; grid-template-columns:repeat(auto-fit,minmax(min(250px,100%),340px)); align-items:start; justify-content:center; gap:12px; border:0; background-color:#12161a; background-image:linear-gradient(to right,rgba(255,255,255,.025) 1px,transparent 1px),linear-gradient(to bottom,rgba(255,255,255,.025) 1px,transparent 1px); background-size:calc(100% / 12) 100%,100% 42px; }
        .combined-building-new .combined-floor-content > .area-group { position:relative; grid-column:auto !important; grid-row:auto !important; min-width:0; width:100%; margin:0; align-self:start; }
        .combined-building-new .area-group { flex:none; }

        /* Parent/child hierarchy: intrinsic columns with a compact, alignment-safe marker. */
        .combined-building-new .area-child-grid,
        .combined-building-new .nested-area-group > .area-child-grid { display:grid; grid-template-columns:1fr; gap:8px; width:100%; box-sizing:border-box; margin:8px 0 0; padding:0; border:0; background:transparent; }
        .combined-building-new .area-child-branch { position:relative; min-width:0; width:100%; box-sizing:border-box; padding-left:18px; }
        .combined-building-new .area-child-branch::before,
        .combined-building-new .nested-area-group > .area-child-grid .area-child-branch::before { content:"↳" !important; display:block !important; position:absolute; left:1px; top:13px; width:12px; height:auto; border:0 !important; color:var(--muted); font:700 11px/1 ui-monospace,monospace; pointer-events:none; z-index:1; }
        .combined-building-new .area-child-branch::after,
        .combined-building-new .nested-area-group > .area-child-grid .area-child-branch::after { display:none !important; content:none !important; }
        .combined-building-new .nested-area-group { width:100%; min-width:0; box-sizing:border-box; }
        .combined-building-new .parent-toggle { position:relative; z-index:20; }

        /* Rails use one exact X and one exact header-junction Y. Dark = connected, light = live power. */
        .flow-rail-cell { position:relative; min-width:0; --rail-base:rgba(230,176,75,.24); --rail-live:#f0c96c; --rail-left:calc(50% - (var(--rail-width) / 2)); }
        .flow-rail-cell::before,
        .flow-rail-cell.active::after { content:""; position:absolute; width:var(--rail-width); left:var(--rail-left); background:var(--rail-base); }
        .flow-rail-cell.active::after { background:var(--rail-live); }
        .flow-rail-cell.connected::before,
        .flow-rail-cell.connected.active::after { top:0; bottom:0; }
        /* Live colour stops exactly at the junction of the last currently active floor. */
        .electric-rail-cell.connected.active.live-end::after { top:0; bottom:auto; height:var(--junction-y); }
        .thermal-rail-cell.connected.active.live-end::after { top:var(--junction-y); bottom:0; }

        /* Electric: source above, exact stop at final floor junction. */
        .electric-rail-cell.connected.last::before,
        .electric-rail-cell.connected.last.active::after { top:0; bottom:auto; height:var(--junction-y); }

        /* Thermal: source below, exact stop at top floor junction. */
        .thermal-rail-cell.connected.first::before,
        .thermal-rail-cell.connected.first.active::after { top:var(--junction-y); bottom:0; }

        .thermal-rail-cell { --rail-base:rgba(93,159,198,.26); --rail-live:#86c4e8; }
        .flow-branch { position:absolute; top:calc(var(--junction-y) - (var(--rail-width) / 2)); height:var(--rail-width); background:var(--rail-base); }
        .electric-branch { left:var(--rail-left); right:0; }
        .thermal-branch { left:0; right:var(--rail-left); }
        .flow-branch.active { background:var(--rail-live); }

        /* Entry elbows use the exact same rail X and junction Y as the building cells. */
        .combined-entry-new { height:44px; display:grid; grid-template-columns:var(--route-gutter) minmax(0,1fr) var(--route-gutter); position:relative; --rail-base:rgba(230,176,75,.24); --rail-live:#f0c96c; --left-rail-x:calc(var(--route-gutter) / 2); --right-rail-x:calc(100% - (var(--route-gutter) / 2)); }
        .combined-entry-new i { position:absolute; display:block; background:var(--rail-base); }
        .combined-entry-new.active i { background:var(--rail-live); }
        .combined-entry-new .entry-center,
        .combined-entry-new .entry-rail { width:var(--rail-width); }
        .combined-entry-new .entry-center { left:calc(50% - (var(--rail-width) / 2)); }
        .combined-entry-new .entry-run { height:var(--rail-width); top:calc(var(--junction-y) - (var(--rail-width) / 2)); }
        .combined-entry-new.thermal { --rail-base:rgba(93,159,198,.26); --rail-live:#86c4e8; }
        .combined-entry-new.electric .entry-center { top:0; height:var(--junction-y); }
        .combined-entry-new.electric .entry-run { left:calc(var(--left-rail-x) - (var(--rail-width) / 2)); width:calc(50% - var(--left-rail-x) + var(--rail-width)); }
        .combined-entry-new.electric .entry-rail { left:calc(var(--left-rail-x) - (var(--rail-width) / 2)); top:var(--junction-y); bottom:0; }
        .combined-entry-new.thermal .entry-rail { left:calc(var(--right-rail-x) - (var(--rail-width) / 2)); top:0; height:var(--junction-y); }
        .combined-entry-new.thermal .entry-run { left:calc(50% - (var(--rail-width) / 2)); width:calc(var(--right-rail-x) - 50% + var(--rail-width)); }
        .combined-entry-new.thermal .entry-center { top:var(--junction-y); bottom:0; }

        /* SYSTEM is completely static: legacy wire animation and every old dash overlay are hidden. */
        .new-system-layout.flow-static .wire.active::after,
        .new-system-layout.flow-static .flow-segment::after,
        .new-system-layout.flow-static .route-svg-path.route-active,
        .new-system-layout.flow-static .entry-svg-flow.active,
        .new-system-layout.flow-static [class*="flow-pattern"],
        .new-system-layout.flow-static .wire::after { animation:none !important; display:none !important; content:none !important; }

        /* Live rail state is recalculated on every HA state/calculation update via normal rerender. */
        .combined-building-new .root-area-group > .area-tile { flex-basis:auto; min-height:92px; height:auto; overflow:visible; }
        .combined-building-new .area-tile { height:auto; overflow:visible; }
        .combined-building-new .area-tile-power { margin-top:8px; padding-top:0; line-height:1.15; overflow:visible; }
        .combined-building-new .area-tile-energy { flex:0 0 auto; }
        .combined-building-new .area-hierarchy-meta { flex:0 0 auto; }
        .combined-building-new .area-group.is-expanded { align-self:start; }

        /* Thermal distribution title is centered exactly like the electrical bus. */
        .new-system-layout .thermal-distribution { position:relative; min-height:46px; box-sizing:border-box; align-items:center; justify-content:center; text-align:center; }
        .new-system-layout .thermal-distribution > span { display:block; width:100%; text-align:center; }
        .new-system-layout .thermal-distribution > strong { position:absolute; right:18px; top:50%; transform:translateY(-50%); }

        .direct-room-zone { margin-top:20px; padding-top:10px; border-top:1px solid var(--line); }
        .direct-room-zone .room-label { color:var(--active); }
        @media (max-width: 760px) {
          .new-system-layout { --route-gutter:34px; }
          .combined-floor-header { align-items:flex-start; flex-direction:column; padding:10px 12px; }
          .floor-meta { justify-content:flex-start; }
          .combined-building-new .combined-floor-content { grid-template-columns:1fr; padding:10px; gap:10px; }
          .combined-building-new .area-child-grid { grid-template-columns:1fr; }
          .new-system-layout .thermal-distribution > strong { position:static; transform:none; margin-left:8px; }
        }
      </style>`;
  }
}

if (!customElements.get("energy-system-dashboard-panel-v0710")) {
  customElements.define("energy-system-dashboard-panel-v0710", EnergySystemDashboardPanel);
}
